...
attributionClassifier = sys.argv[2]
classesLabler = sys.argv[3]
clf = pickle.load(open(attributionClassifier, 'rb'))
authorslabeler = LabelEncoder()
authorslabeler.classes_ = np.load(classesLabler)
...
def getDocText(doctype, filePath):
    if doctype == "original":
        ...
        try:
            ...
            inputText = io.open(filePath + "/BestChangeInIteration_" + str(getMaxIterationNumberAvailable(filePath)), "r", errors="ignore").readlines()
            inputText = inputText[:-1]
            inputText = ''.join(str(e) + "" for e in inputText)
            return inputText
...
for file in fileNames:
    ...
    for run in allRuns:
        ...
        try:
            meteor_score = getMeteorScore(run)
        except Exception as e:
            exceptionCounter+=1
            print(e, exceptionCounter)
            continue
    ...
    if obfuscated_runs:
        ...
        runName, bestMeteor = getBestRun(obfuscated_runs)
        final_text = getDocText('obfuscated', runName)
    else:
        totalObfuscated += 1
        runName, bestMeteor = getBestRun(nonObfuscated_runs)
        final_text = getDocText('obfuscated', runName)

    writeObfuscatedText = open("Obfuscated_Documents" + "/" + str(file.split('/')[-1]), "w")
    writeObfuscatedText.write(final_text)
    writeObfuscatedText.close()

    meteorScores.append(bestMeteor)


print("Total Non Obfuscated : ", totalObfuscated)
print("Average METEOR : ", np.mean(meteorScores))
