...
def getWordNeighboursDictionary(inputText,totalNeighbours):
    wordVectorModel = word2vec.KeyedVectors.load_word2vec_format("../common/Word2vec.bin",
                                                                 binary=True)
    average_time = []
    word_neighbours = {}
...
def main():
    ...
    parser.add_argument("--indexNumber", "-run", help="Number of run", default=1)
    ...
    indexNumber = int(args.indexNumber)
    allowedNeighbours = int(args.allowedNeighbours)
    classifierType = args.classifierType

    runNumber, documentNumber = getRunAndDocumentNumber(indexNumber,datasetName,authorstoKeep)
    documentNumber -=1
    ####################################################
    ...
    clf.loadClassifier()
    testInstancesFilename = "../datasets/" + str(datasetName) + '-' + str(authorstoKeep) + '/X_test.pickle'
    ...
    runNumber = str(indexNumber) + "_" + str(runNumber)+ "_" + str(time.time())
    ...
    qualitativeoutputResults = open(qualitative_results_folder + "/Original_Text", "w")
...
#    while (iteration_m < mutant_x.iterations) and (obfuscated == False) :
    while (iteration_m < mutant_x.iterations) :
